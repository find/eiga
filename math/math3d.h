...
inline quat mat3::toQuat() const
{
    ...
    if (tr > 0) {
        ...
        real_t s = sqrt(1.0 + m[0][0] - m[1][1] - m[2][2]) * 2; // s=4*qx
        q.w = (m[2][1] - m[1][2]) / s;
        q.x = real_t(0.25 * s);
        q.y = (m[0][1] + m[1][0]) / s;
        q.z = (m[0][2] + m[2][0]) / s;
    } else if (m[1][1] > m[2][2]) {
        real_t s = sqrt(1.0 + m[1][1] - m[0][0] - m[2][2]) * 2; // s=4*qy
        q.w = (m[0][2] - m[2][0]) / s;
        q.x = (m[0][1] + m[1][0]) / s;
        q.y = real_t(0.25 * s);
        q.z = (m[1][2] + m[2][1]) / s;
    } else {
        real_t s = sqrt(1.0 + m[2][2] - m[0][0] - m[1][1]) * 2; // s=4*qz
        q.w = (m[1][0] - m[0][1]) / s;
        q.x = (m[0][2] + m[2][0]) / s;
        q.y = (m[1][2] + m[2][1]) / s;
        q.z = real_t(0.25 * s);
...
struct mat4 {
    ...
    inline void flipViewHand() { row(0) = -row(0); row(2) = -row(2); }
    inline void flipProjHand() { mat4 t = transposed(); t.row(0) = -t.row(0); t.row(2) = -t.row(2); *this = t.transposed(); }
